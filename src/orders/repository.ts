...
export default {
  setOrder: async (body: any) => {
    ...
    await connection.transaction(async (transaction) => {
      const detail = await Orders.create(body, { raw: true, transaction });
      ...
      await OrderDetails.create({ order_code: orderCode, order_state: "주문완료", payment_state: "결제진행중", order_detail_id: detail.id }, { raw: true, transaction });
    }).catch((err) => {
      throw err;
    })
  },
